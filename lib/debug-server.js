...
DebugServer.prototype.start = function(options) {
  ...
  // add for require resources
  // have to use /socket.io/debug as path for some reasons
  app.get('/socket.io/debug', debugAction.bind(this));
  app.get('/socket.io/debug/*', (req, res, next) => {
    var url = req.url;
    require('request').get(url.replace(/.*\/debug/, `http://127.0.0.1:${this._config.webPort}`)).pipe(res);
  });
